...
from arturo.commands.build import Cmd_preprocess, Cmd_source_headers, Cmd_header_libs, Cmd_source_files, Cmd_d_to_p, Cmd_mkdirs, Cmd_source_libs, Cmd_lib_source_files, Cmd_lib_source_headers
...
def getAllCommands():
    ...
    if not hasattr(module, "_commands"):
        ...
        module._commands = {Command.command_class_to_commandname(
            commandClass): commandClass for name, commandClass in commands}  # @UnusedVariable
        ...
        if issubclass(commandClass, ProjectCommand):
            ...
            if issubclass(commandClass, ConfiguredCommand):
                ...
                command = commandClass(
                    environment, project, project.getLastConfiguration(), console)
            else:
