...
class Command(ArgumentVisitor, Runnable):
    ...
    def getAllCommands(self):
        ...
        filteredMembers = inspect.getmembers(
            commandsModule, lambda x: (isfunction(x) and x.__name__ is "getAllCommands"))
    ...
    def _appendCommandNameForSelfRecursive(cls, outTemplates):
        ...
        try:
            outTemplates['commands'][
                commandName] = __app_name__ + " " + commandName
        except KeyError:
            outTemplates['commands'] = {
                commandName: __app_name__ + " " + commandName}
