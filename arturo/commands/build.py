...
from arturo.parsers import InferLibraryDependenciesFromSourceParser
...
class Cmd_preprocess(ConfiguredCommand):
    ...
    def __init__(self, environment, project, configuration, console):
        super(Cmd_preprocess, self).__init__(
            environment, project, configuration, console)
    ...
    def onVisitArgParser(self, parser):
        parser.add_argument(
            "--output_dir", default=self.getConfiguration().getBuilddir())
...
class Cmd_source_headers(ConfiguredCommand):
    ...
    def getAllHeaders(self, dirnameonly=False):
        configuration = self.getConfiguration()
        headers = configuration.getHeaders(dirnameonly=dirnameonly)
        core = configuration.getBoard().getCore()
        headers += core.getHeaders(dirnameonly=dirnameonly)
        variant = configuration.getBoard().getVariant()
        headers += variant.getHeaders(dirnameonly=dirnameonly)
        # TODO: get all libraries and foreach get library headers.
        return headers
    ...
    def run(self):
        ...
        for header in headers:
            headerFolders.add(
                os.path.relpath(os.path.dirname(header), projectPath))
...
class Cmd_source_files(ConfiguredCommand):
    ...
    def __init__(self, environment, project, configuration, console):
        super(Cmd_source_files, self).__init__(
            environment, project, configuration, console)
        self._sources = None
    ...
    def getAllSources(self):
        if self._sources is None:
            ...
            projectPath = self.getProject().getPath()
            self._sources = [
                os.path.relpath(sources[x], projectPath).strip() for x in range(len(sources))]
...
class Cmd_lib_source_files(ConfiguredCommand):
    ...
    def run(self):
        ...
        if not library:
            raise RuntimeError(_("Version {} of library {} was not available.".format(
...
class Cmd_lib_source_headers(ConfiguredCommand):
    ...
    def getLibraryHeaders(self, library, inoutheaderset=set(), dirnameonly=True):
        configuration = self.getConfiguration()

        libraries = configuration.getLibraries()
        libNameAndVersion = Library.libNameAndVersion(library)
        libraryVersions = libraries.get(libNameAndVersion[0])
        if not libraryVersions:
            raise RuntimeError(
                _("No library with name {} was found.".format(libNameAndVersion[0])))
        ...
        inoutheaderset.update(library.getHeaders(dirnameonly=dirnameonly))
        dependantLibs = self._findAllLibrariesForLibrary(library)
        for dependantLib in dependantLibs.itervalues():
            inoutheaderset.update(
                set(dependantLib.getHeaders(dirnameonly=dirnameonly)))
        core = configuration.getBoard().getCore()
        inoutheaderset.update(set(core.getHeaders(dirnameonly=dirnameonly)))
        variant = configuration.getBoard().getVariant()
        inoutheaderset.update(set(variant.getHeaders(dirnameonly=dirnameonly)))
        return inoutheaderset
    ...
    # +-----------------------------------------------------------------------+
    def run(self):
        self.getConsole().stdout(*self.getLibraryHeaders(self._library))
    ...
    def _findAllLibrariesForLibrary(self, library):
        ...
        setattr(
            self, "_libdeps", cmd_source_libs.findAllLibrariesForLibrary(library))
        return self._libdeps
...
class Cmd_source_libs(Cmd_source_headers, Cmd_source_files):
    ...
    def __init__(self, environment, project, configuration, console):
        super(Cmd_source_headers, self).__init__(
            environment, project, configuration, console)
        self._librariesFromSourceParser = InferLibraryDependenciesFromSourceParser(
            console)
        self._libdeps = None
    ...
    def getAllPossibleLibsForProject(self):
        ...
        for source in sources:
            self._libdeps.update(self._librariesFromSourceParser.getPossibleLibsForSource(
                self.getConfiguration(), source))
        headers = self.getAllHeaders()
        for header in headers:
            self._libdeps.update(self._librariesFromSourceParser.getPossibleLibsForSource(
                self.getConfiguration(), header))
    ...
    def findAllLibrariesForLibrary(self, library):
        results = dict()
        self._findAllLibrariesRecursive(
            library, results, excludeHeaderOnly=False)
        return results
    ...
    def run(self):
        ...
        if console.willPrintDebug():
            console.printDebug("Project {} has {} library dependencies".format(
                self.getProject().getName(), len(libdeps)))
    ...
    def _findAllLibrariesRecursive(self, library, libdeps, excludeHeaderOnly=True):
        ...
        for header in libraryHeaders:
            headerLibDeps = self._librariesFromSourceParser.getPossibleLibsForSource(self.getConfiguration(),
                                                                                     header, excludeHeaderOnly=excludeHeaderOnly)
            before = len(libdeps)
            libdeps.update(headerLibDeps)
            if len(libdeps) > before:
                for headerLibDep in headerLibDeps.itervalues():
                    if console.willPrintVerbose():
                        console.printVerbose("Library {} depends on library {}".format(
                            library.getName(), headerLibDep.getName()))
                    self._findAllLibrariesRecursive(
                        headerLibDep, libdeps, excludeHeaderOnly=excludeHeaderOnly)
        librarySources = library.getSources()
        for source in librarySources:
            sourceLibDeps = self._librariesFromSourceParser.getPossibleLibsForSource(self.getConfiguration(),
                                                                                     source, excludeHeaderOnly=excludeHeaderOnly)
            before = len(libdeps)
            ...
            if len(libdeps) > before:
                for sourceLibDep in sourceLibDeps.itervalues():
                    if console.willPrintVerbose():
                        console.printVerbose("Library {} depends on library {}".format(
                            library.getName(), sourceLibDep.getName()))
                    self._findAllLibrariesRecursive(
                        sourceLibDep, libdeps, excludeHeaderOnly=excludeHeaderOnly)
...
class Cmd_header_libs(Cmd_source_libs):
    ...
    def run(self):
        libdeps = self.getAllPossibleLibsForProject()
        headers = set()
        headers.update(self.getAllHeaders(dirnameonly=True))
        cmd_lib_source_headers = self.getCommand("Cmd_lib_source_headers")
        for libdep in libdeps:
            headers.update(
                cmd_lib_source_headers.getLibraryHeaders(libdep, inoutheaderset=headers, dirnameonly=True))
...
class Cmd_d_to_p(ConfiguredCommand):
    ...
    def run(self):
        ...
        ppath = re.sub("\.d$", '.' + Cmd_d_to_p.PFILE_EXTENSION,
                       dpath) if self._ppath is None else self._ppath
    ...
    def _getLibDeps(self, ppath):
        ...
        with open(ppath, "r") as pfile:
            ...
            for line in pfile:
                if endswithColon.search(line):
                    nameAndVersion = Library.libNameAndVersion(
                        os.path.basename(os.path.dirname(os.path.realpath(line[:-1]))))
                    if nameAndVersion in possibleLibraryDependencies:
                        versions = possibleLibraryDependencies[
                            nameAndVersion[0]]
                    else:
                        versions = set()
                        possibleLibraryDependencies[
                            nameAndVersion[0]] = versions
        ...
        for libname, library in libraries.iteritems():
            if libname in possibleLibraryDependencies:
                ...
                if len(versions) > 1:
                    raise RuntimeError(_("{} required {} different versions of the {} library.".format(
                        os.path.basename(ppath), len(versions), libname)))
                libnames.append("{}.{}".format(Library.libNameFromNameAndVersion(
                    library.getName(), versions.pop()), self.LIBDEP_EXTENSION))
        return libnames
