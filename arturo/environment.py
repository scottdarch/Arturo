...
class Configuration(object):
    ...
    def getBuilddir(self):
        if self._builddir is None:
            self._builddir = os.path.join(
                self._project.getBuilddir(), self._boardName)
...
class ProjectSourceRootAggregator(Arduino15PackageSearchPathAgent):
    ...
    def __init__(self, project, console, exclusions=None):
        super(ProjectSourceRootAggregator, self).__init__(
            SearchPath.ARTURO2_SOURCE_FILEEXT, console, exclusions=exclusions, followLinks=True)
        self._project = project
...
class Project(object):
    ...
    def getBuilddir(self):
        if self._builddir is None:
            self._builddir = os.path.join(
                self._path, SearchPath.ARTURO2_BUILDDIR_NAME)
        return self._builddir
    ...
    def getLastConfiguration(self):
        ...
        mergedPreferences = MakefilePropertyParser.parse(os.path.join(
            self._path, JinjaTemplates.TEMPLATES['makefile']), preferences, self._console)
    ...
    def getSourceRoots(self, treatAsSourceFolders=None):
        ...
        if treatAsSourceFolders is not None:
            for forcedSource in treatAsSourceFolders:
                forcedSourceProjectPath = os.path.join(
                    self._path, forcedSource)
    ...
    def getLibraries(self):
        if self._libraries is None:
            # treat anything in a "library" folder under the project path as a
            # library
            projectPath = self.getPath()
            forceTreatAsLibraries = list()
            for folderName in SearchPath.ARDUINO15_LIBRARY_FOLDER_NAMES:
                forceTreatAsLibraries.append(
                    os.path.join(projectPath, folderName))
            self._libraries = self.getEnvironment().getLibrariesFor(
                projectPath, forceTreatAsLibraries)
...
class Environment(object):
    ...
    def getPreferences(self):
        if self._preferences is None:
            self._preferences = Preferences(
                self.getSearchPath(), self.getConsole())
    ...
    def getLibraries(self):
        ...
        if self._libraryIndex is None:
            ...
            for meta_data in self._getLibraryMetadata().itervalues():
                ...
                library = Library(
                    libraryName, self, self._console, libraryVersion)
                if libraryName not in self._libraryIndex.keys():
            ...
            for searchPath in self.getSearchPath().getPaths():
                self._libraryIndex.update(
                    self.getLibrariesFor(searchPath, continueOnError=True))
            ...
            if os.path.isdir(librariesDir):
                for item in os.listdir(librariesDir):
                    ...
                    if os.path.isdir(libraryDir):
                        try:
                            library = Library.fromDir(
                                self, libraryDir, self._console, platform)
                            ...
                            if forcedLibrariesPaths:
                                ...
                                if os.path.commonprefix(prefixList) in forcedLibrariesPaths:
                                    # Any folder under a "forced path" is
                                    # considered a library.
                                    library = Library(os.path.basename(
                                        libraryDir), self, self._console, libraryPath=libraryDir, libraryPlatform=platform)

                            elif continueOnError:
                                self.getConsole().printDebug(
                                    _("{} was not a well-formed library.".format(libraryDir)))
                                continue
                            else:
                                raise e
                        if library.getName() not in libraries.keys():
                            libraries[library.getName()] = dict()
                        libraries[
                            library.getName()][library.getVersion()] = library
    ...
    def _getLibraryMetadata(self):
        ...
        libraryMetadataPath = self.getSearchPath().findFirstFileOfNameOrThrow(
            Environment.LIBRARY_INDEX_NAMES, 'library index')

        # the package folders are found under a folder ARDUINO15_PACKAGES_PATH
        # next to the packages index file
        if self._packageRootPath is None:
            self._packageRootPath = os.path.join(
                os.path.dirname(libraryMetadataPath), SearchPath.ARDUINO15_PACKAGES_PATH)
        ...
        for libraryMetadata in libraryMetadataList:
            library_name_and_version = Library.libNameFromNameAndVersion(
                libraryMetadata['name'], libraryMetadata['version'])
            self._libraryMetadataIndex[
                library_name_and_version] = libraryMetadata
    ...
    def _getPackageMetadata(self):
        ...
        packageMetadataPath = self.getSearchPath().findFirstFileOfNameOrThrow(
            Environment.PACKAGE_INDEX_NAMES, 'package index')

        # the package folders are found under a folder ARDUINO15_PACKAGES_PATH
        # next to the packages index file
        if self._packageRootPath is None:
            self._packageRootPath = os.path.join(
                os.path.dirname(packageMetadataPath), SearchPath.ARDUINO15_PACKAGES_PATH)
        ...
        for packageMetadata in packageMetadataList:
            self._packageMetadataIndex[
                packageMetadata['name']] = packageMetadata
