    ...
    def askPickOneFromList(self, prompt, optionList, responseList=None):
        ...
        for x in range(listLen):
            if len(optionList[x]) > 1:
                listAsString = listAsString + (self._indent + Console.INDENTATION + _(
                    "{} : {} - {}".format(str(x + 1), optionList[x][0], optionList[x][1])) + os.linesep)
            else:
                listAsString = listAsString + \
                    (self._indent + Console.INDENTATION +
                     _("{} : {}".format(str(x + 1), optionList[x][0])) + os.linesep)

        commentAndList = _(
            "{0}{1}".format(listAsString, self._indent + prompt + os.linesep))
        responseAsInt = -1
        while True:
            ...
            if responseAsInt < 0 or responseAsInt >= listLen:
                self._printMessage(
                    _("Please enter a number between {} and {}.".format(str(1), str(listLen))))
            else:
                break
