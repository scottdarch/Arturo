...
class Package(object):
    ...
    def __init__(self, environment, rootPath, searchPath, console, packageMetaData):
        ...
        self._hardwareDir = os.path.join(
            self._packagePath, SearchPath.ARDUINO15_HARDWARE_PATH)
    ...
    def getToolChains(self):
        if not self._toolChainsDict:
            ...
            for toolchainMetadata in self._packageMetadata['tools']:
                ...
                toolchainVersionsDict[toolchainVersionName] = ToolChain(
                    self, toolchainMetadata, self._console)
            # Now go back through and sort the verion collections
            for name, versions in self._toolChainsDict.iteritems():
                self._toolChainsDict[name] = OrderedDict(
    ...
    def getToolChainByNameAndVerison(self, nameAndVersion):
        ...
        if lastHyphen == -1:
            raise ValueError(
                "{} is not a hyphenated name-value string.".format(nameAndVersion))
    ...
    def getPlatforms(self):
        if self._platformIndex is None:
            ...
            for platformMetadata in self._packageMetadata['platforms']:
                platform = Platform.ifExistsPlatform(
                    self, self._hardwareDir, self._searchPath, self._console, platformMetadata)
                if platform:
                    if self._console:
                    ...
                    self._platformIndex[
                        platformMetadata['architecture']] = platform
                else:
                    # TODO: store missing platforms for a future
                    # "download-platform" command.
                    if self._console:
