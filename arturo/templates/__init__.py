...
class JinjaTemplate(object):
    ...
    def __init__(self, jinjaEnvironment, templateName, templateFileName, parentTemplateName=None):
        ...
        self._parentTemplate = parentTemplateName + \
            self.JINJASUFFIX if parentTemplateName is not None else None
        self._template = None
        self._jinjaEnv = jinjaEnvironment

    def getTemplate(self):
        if self._template is None:
            self._template = self._jinjaEnv.get_template(
                self._fileName + self.JINJASUFFIX, parent=self._parentTemplate)
...
class JinjaTemplates(object):
    ...
    def createJinjaEnvironmentForTemplates(cls):
        jinjaEnv = jinja2.Environment(
            loader=PackageLoader(__lib_name__, cls.getRelPathToTemplatesFromPackage()))
